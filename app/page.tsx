...
export default function Home() {
  ...
  // 👑 STEP 1: กำหนด Email ของ Admin (เปลี่ยนเป็นเมลที่คุณโอ๊ตใช้ล็อกอิน)
  ...
  // ฟังก์ชันลบกระทู้ (เรียกใช้ API Step 2)
  ...
  // ฟังก์ชันลบคอมเมนต์ (เรียกใช้ API Step 3)
  ...
  // --- ฟังก์ชันอื่นๆ (Like, Comment, AI, Update) ---
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    await fetch(`/api/posts/${editingPost.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: editTitle, content: editContent }) });
    setEditingPost(null);
    fetchPosts();
  };
  ...
  return (
    <main className="p-8 max-w-4xl mx-auto min-h-screen text-gray-900 bg-gray-50">
      ...
      {/* ฟอร์มโพสต์ใหม่ */}
      {session && (
        <form onSubmit={handlePost} className="mb-12 bg-white p-6 rounded-xl border shadow-sm">
          ...
          <button type="submit" className="bg-green-600 text-white px-8 py-2 rounded-lg font-bold">โพสต์เลย</button>
        </form>
      )}
      ...
      <div className="space-y-8">
        {filteredPosts.map((post: any) => (
          <div key={post.id} className="border p-6 rounded-xl bg-white shadow-sm">
            <div className="flex justify-between items-start mb-2">
              ...
              {/* ตรวจสอบสิทธิ์ Admin/Owner เพื่อแสดงปุ่มลบ */}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50">
          <div className="bg-white p-8 rounded-2xl w-full max-w-lg shadow-2xl">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button onClick={handleUpdate} className="px-6 py-2 bg-green-600 text-white rounded-lg font-bold">บันทึก</button>
            </div>
          </div>
        </div>
      )}
